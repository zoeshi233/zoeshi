...
public class Lab1Tester {
	...
	public void test04_removeLastTwoDigits() {
		for (int n = 0; n < 100; n++) {
			String error = String.format("removeLastTwoDigits(%d) failed", n);
			assertEquals(error, 0, Lab1.removeLastTwoDigits(n));
		}
	}
	...
	public void test14_flipBit() {
		...
		assertEquals(error, 1, Lab1.flipBit(x));
	}
	...
	public void test15_flipBit() {
		int x = 1;
		String error = String.format("flipBit(%s) failed", x);
		assertEquals(error, 0, Lab1.flipBit(x));
	}
	...
	private static char randomChar() {
		return (char) (Lab1Tester.rng.nextInt('Z' - 'A') + 'A');
	}
	...
	public void test22_middleChar() {
		...
		int mid = 0;
		builder.append(Lab1Tester.randomChar());
	...
	private static Pair randomString(int n) {
		...
		for (int i = 0; i < n; i++) {
			char c1 = Lab1Tester.randomChar();
			char c2 = Lab1Tester.randomChar();
			char lower = Character.toLowerCase(c1);
			char upper = c2;
			exp.append(lower);
			in.append(Lab1Tester.randomCase(lower));
			if (i < n - 1) {
				exp.append(upper);
				in.append(Lab1Tester.randomCase(upper));
			}
		}
	...
	public void test23_alternatingCaps() {
		for (int length = 1; length < 100; length++) {
			for (int i = 0; i < 10; i++) {
				Pair testCase = Lab1Tester.randomString(length);
